...
- **One-click export:** copy YAML, download `spec.yaml`, or download the full kit
  `.zip` (spec + README + `files/` tree + publish workflow + Docker Hub push script).
...
## Publishing a kit to Docker Hub

**The push script is not in this repo — it's generated inside each kit you export.**
Download a kit `.zip`, unzip it, and you'll find `scripts/push-kit.sh` at the kit
root (generated by `lib/kit-export.ts`, along with `.github/workflows/publish.yml`).
...
The script runs `sbx kit push . <ref>:<version>` (reusing your existing `docker
login` credentials) and prints the `sbx kit inspect` command to resolve the
published digest — which is what consumers must pin to (`oci://…@sha256:<digest>`;
tags are rejected for consumption). Pass a full ref with a `/` (e.g.
`ghcr.io/org/<kit>`) to target a registry other than Docker Hub. Tagging `v*` also
publishes via the bundled workflow once you set the `DOCKERHUB_USERNAME` /
`DOCKERHUB_TOKEN` secrets. Requires the [sbx CLI](https://github.com/docker/sbx-kits-contrib).
